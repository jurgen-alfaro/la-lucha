...
function GradientePhotos({ photo }) {
  const { deleteGradientePhoto } = useContext(QuiebraGradientesContext);
  const [photoId, setPhotoId] = useState(photo.idphoto);

  const handleClick = (e) => {
    e.preventDefault();
    deleteGradientePhoto(photoId);
  };
...
export default GradientePhotos;
